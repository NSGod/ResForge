//
//  ViewController_head[bhed].swift
//  FontEditor
//
...
/// This file handles both the UI ViewControllers for the `head` table, which is
/// the primary required table for a font that uses outlines (`glyf`, `CFF `, `CFF2`)
/// as well as for the `bhed` table, which is the analogous primary required table for
/// bitmap-only fonts (`bdat`, `EBSC`, etc.)

final class ViewController_bhed: ViewController_abstractHead {
    var table:          FontTable_bhed

    required init?(with fontTable: FontTable) {
        self.table = fontTable as! FontTable_bhed
    ...
    override var nibName: NSNib.Name? {
        return "ViewController_head"
    }

    override var representedObject: Any? {
        didSet {
            self.table = self.representedObject as! FontTable_bhed
        }
    }

    override func prepareToSave() throws {
        try super.prepareToSave()
    ...
    @IBAction override func now(_ sender: Any) {
...
final class ViewController_head: ViewController_abstractHead {
...
class ViewController_abstractHead: FontTableViewController, NSControlTextEditingDelegate {
    @IBOutlet weak var flagsControl:        BitfieldControl!
    @IBOutlet weak var macStyleControl:     BitfieldControl!

    var _table:                             FontTable
    ...
    deinit {
        ...
        Self.tableKeyPaths.forEach { _table.removeObserver(self, forKeyPath: $0, context: &Self.tableContext) }
    ...
    override func viewDidLoad() {
        ...
        Self.tableKeyPaths.forEach { _table.addObserver(self, forKeyPath: $0, options: [.new, .old], context: &Self.tableContext) }
        Self.keyPaths.forEach { addObserver(self, forKeyPath: $0, options: [.new, .old], context: nil) }
    }

    override var representedObject: Any? {
        didSet {
            self._table = self.representedObject as! FontTable
        }
    }
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if context == &Self.tableContext {
            undoManager?.registerUndo(withTarget: self, handler: {
                $0._table.setValue(change![.oldKey], forKey: keyPath)
            })
        } else {
            undoManager?.registerUndo(withTarget: self, handler: {
