...
struct POST {
    ...
    static func postResources(from resources: [Resource]) throws -> [POST] {
    ...
    static func data(from postResources: [POST], outputFormat: PostScriptType1FontFile.Format = .ascii) throws -> Data? {
        if outputFormat == .binary {
            fatalError("Binary PostScript not supported")
        }
        var mString = ""
